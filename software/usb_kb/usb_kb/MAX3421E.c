...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	BYTE bussin[2] = {(reg+2), val};
	if (alt_avalon_spi_command(SPI_0_BASE, 0, 2, &bussin, 0, 0, 0) < 0) {
		printf("an error");
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	//return (data + nbytes);
	BYTE bussin[nbytes+1];
	bussin[0] = reg+2;
	int n;
	for(n = 0; n < nbytes; n++) {
		bussin[n+1] = data[n];
	}
	if (alt_avalon_spi_command(SPI_0_BASE, 0, nbytes+1, &bussin, 0, 0, 0) < 0) {
			printf("an error");
	}
	return (data + nbytes);
}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	//return val
	BYTE val;
	if(alt_avalon_spi_command(SPI_0_BASE, 0, 1, &reg, 1, &val, 0) < 0) {
		printf("an error");
	}
	return val;
}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	//return (data + nbytes);
	if(alt_avalon_spi_command(SPI_0_BASE, 0, 1, &reg, nbytes, data, 0) < 0) {
		printf("an error");
	}
	return (data + nbytes);
}
